...
export async function saveFileToBucket(
  ...
  fileBuffer: Buffer, 
  originalName: string,
  fileType: 'background' | 'avatar' | 'attachment' = 'attachment'
...
export async function getUserFiles(userId: number, fileType?: string): Promise<any[]> {
  let query = db.query.userFiles.findMany({
    where: eq(userFiles.userId, userId)
  });
  
  if (fileType) {
    query = db.query.userFiles.findMany({
      where: (userFiles, { eq, and }) => 
        and(eq(userFiles.userId, userId), eq(userFiles.fileType, fileType))
    });
  }
