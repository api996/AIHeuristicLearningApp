...
2. 严格评估每个来源的质量和可信度，使用以下标准：
   - 高质量来源(8-10分)：学术论文(如arXiv)、科学期刊、大学网站(.edu、.ac域名)、政府机构(.gov域名)、知名研究机构、专业技术文档、科研出版物
   - 中等质量来源(5-7分)：知名新闻媒体(如纽约时报、华尔街日报、BBC、路透社、财新、新华社、澎湃新闻等)、行业权威网站、知名企业官网(.com/.org官方网站)、专业博客
   - 低质量来源(1-4分)：社交媒体帖子、个人网站、二手信息聚合站、缺乏原创内容的复制网站
   - 必须完全排除的来源(0分)：百度百家号、百家号、今日头条号、搜狐号、网易号、大风号、企鹅号、一点号等自媒体平台内容；"CSDN博客"中纯复制内容；包含大量广告的网站；标题党网站；无明确作者或机构的内容农场
...
重要指导原则 - 请严格遵循：
1. 必须优先选择学术、专业和权威来源(arXiv、知名大学网站、官方研究机构、专业技术文档)
2. 必须完全排除以下来源：
   a) 所有自媒体平台内容：百度百家号(通常URL包含baijiahao.baidu.com)、今日头条号、搜狐号、网易号、大风号、企鹅号、一点号等
   b) CSDN等编程网站上纯复制内容，产品营销号，无明确作者的文章
   c) 内容农场类网站(URL包含大量随机字符或数字)
   d) 问答平台上质量低的回答(如未经验证的百度知道回答)
   e) 纯广告性质的页面，标题党网站，域名中有多个连字符的可疑网站
3. 对于中文搜索结果，必须特别排除常见的低质量媒体聚合网站：
   a) 百度百家号 (baijiahao.baidu.com)
   b) 包含 "toutiao" 的自媒体网站
   c) 网址中包含 "sohu", "163", "ifeng" 等加上 "/a/" 或 "/c/" 路径的纯自媒体内容
4. 确保所选内容与用户真实意图高度相关，而不只是表面上匹配关键词
5. 当可选的高质量来源不足时，可以适当降低筛选标准，但必须在摘要中注明信息可能不够权威可靠`;
  ...
  private async assessContentValue(content: string): Promise<ContentValueAssessment> {
    try {
      ...
      // 构建提示词
      const prompt = `
分析以下内容，严格判断是否包含实质性的教育价值。
评估标准:
1. 信息密度 - 内容包含的信息点数量和质量，是否有具体事实而非空洞表述
2. 知识深度 - 涉及概念的复杂度、专业性和深度，是否有深入解析而非浅尝辄止
3. 教育价值 - 内容对学习者的帮助程度，是否提供了值得学习的知识
4. 来源可信度 - 内容是否来自可靠来源，而非营销号、自媒体或内容农场
5. 原创性 - 内容是否有独特见解，而非简单复制或拼凑

自媒体平台特征（这类内容通常价值较低）:
- URL包含：baijiahao.baidu.com、toutiao、sohu/a/、163/dy、ifeng/c/等
- 内容风格：标题党、过度使用感叹号、缺乏专业术语、缺少引用来源
- 文本特征：大量口语化表达、过度推销产品、缺乏深度分析

内容:
${truncatedContent}

请以JSON格式回复，并对低质量内容要严格评分:
{
  "isValuable": true/false,
  "score": 0到1之间的评分(高质量内容>0.7，中等质量内容0.4-0.7，低质量内容<0.4),
  "reason": "简要解释评分原因，说明哪些因素影响了评分",
  "sourceType": "权威机构/专业网站/内容聚合/营销号/自媒体/无法判断"
}`;
