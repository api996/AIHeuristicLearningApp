...
import { clearLearningPath, directSaveLearningPath } from '../services/learning/directSave';
...
router.get('/:userId', async (req, res) => {
  try {
    ...
    // 如果强制刷新，确保清除现有数据并重新生成
    if (forceRefresh) {
      // 使用新的直接数据库函数清除现有的学习轨迹数据
      await clearLearningPath(userId);
      log(`[API] 已清除用户 ${userId} 的现有学习轨迹数据`);
    }
  ...
  try {
    ...
    if (clusterResult && clusterResult.topics && clusterResult.topics.length > 0) {
      ...
      if (result.topics && result.topics.length > 0) {
        ...
        // 保存更新后的学习轨迹 - 使用直接数据库函数
        try {
          await directSaveLearningPath(
            userId,
            updatedTopics,
