...
/**
 * 强制刷新知识图谱
 * GET /api/learning-path/:userId/force-refresh
 */
router.get('/:userId/force-refresh', async (req, res) => {
  try {
    const userId = utils.safeParseInt(req.params.userId);
    ...
    // 然后生成新的知识图谱（强制刷新）
    const knowledgeGraph = await buildUserKnowledgeGraph(userId, true);
