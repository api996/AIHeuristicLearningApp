 ...
 */
async function main() {
  log('=== 开始批量处理缺失嵌入的记忆 ===', 'info');
  
  try {
    // 获取缺少嵌入的记忆 (处理20条)
    const memories = await getMissingEmbeddings(20);
    
    if (memories.length === 0) {
      log('没有找到需要处理的记忆', 'warning');
      return;
    }
    
    log(`准备处理 ${memories.length} 条记忆，每批处理2条，批次间隔延10秒`, 'info');
    
    // 分批处理记忆，每批只奇2条，批次之间等待更长时间
    const result = await processBatch(memories, 2, 10000);
