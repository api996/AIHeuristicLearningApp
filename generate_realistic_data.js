...
const { memories, memoryEmbeddings } = require('./shared/schema');
...
async function cleanupTestData(userId) {
  ...
  try {
    ...
    const memoryResult = await db.select({ id: memories.id })
      .from(memories)
      .where(db.eq(memories.userId, userId));
    
    const memoryIds = memoryResult.map(m => m.id);
    log(`找到 ${memoryIds.length} 条记忆数据`);
    
    // 首先删除向量嵌入
    const deleteEmbeddingsResult = await db.delete(memoryEmbeddings)
      .where(db.inArray(memoryEmbeddings.memoryId, memoryIds));
    
    log(`已删除 ${memoryIds.length} 条记忆向量嵌入`);
    
    // 然后删除记忆记录
    const deleteMemoriesResult = await db.delete(memories)
      .where(db.eq(memories.userId, userId));
    
    log(`已删除 ${memoryIds.length} 条记忆记录`);
...
async function main() {
  try {
    ...
    if (autoMode) {
      ...
      readline.question(`是否清理用户 ${userId} 的现有数据？(y/n)`, async (answer) => {
        if (answer.toLowerCase() === 'y') {
          // 清理现有数据
          await cleanupTestData(userId);
        }
        
        readline.question(`是否为用户 ${userId} 创建 ${memoryCount} 条新的真实对话记忆？(y/n)`, async (answer2) => {
          readline.close();
